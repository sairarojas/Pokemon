  ...
  <div>
    <nav class="navbar" style="background-color: #edeff0f8;">
      <div class="container-fluid">
        ...
        <input class="form-control me-2 amplio" type="search" placeholder="🔍Buscar nombre de Pokémon"
          aria-label="buscar" style="width: 400px;" v-model="busqueda">
        <button type="button" class="btn btn-success" @click="buscarPokemon">Buscar por nombre</button>
      </form>
       </div>
    </nav>
                ...
                <div class="modal-body">
                  <div class="row">
                    <div class="col-6 col-sm-6">
                      ...
                      <button v-for="(tipo, id) in pokemonSeleccionado.tipo_pk" :key="id"
                        :class="['btn', 'tipo1-color']"
...
export default {
  ...
  setup() {
    ...
    try {
      const response = await axios.get(
        `https://pokeapi.co/api/v2/pokemon/${busqueda.value.toLowerCase()}`
      );
      const data = response.data;
      ...
      // Muestra el Pokémon encontrado en el modal.
      verDetalle(pokemon);
    } catch (error) {
      console.error("No se encontró ningún Pokémon con ese nombre:", error);
